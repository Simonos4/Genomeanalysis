...
#  This script should be executed from unitigging/lferriphilum.ovlStore.BUILDING/, but the binary needs
...
if [ -e ./lferriphilum.ovlStore.BUILDING/$jobname.info -a ! -e ./lferriphilum.ovlStore.BUILDING/$jobname.started ] ; then
  echo "Sorting job finished; info file './lferriphilum.ovlStore.BUILDING/$jobname.info' exists."
  exit
fi
#
#  Sort!
...
$bin/ovStoreSorter \
  -deletelate \
  -O  ./lferriphilum.ovlStore.BUILDING \
  -S ../lferriphilum.seqStore \
  -C  ./lferriphilum.ovlStore.config \
  -f \
  -s $jobid \
  -M 1 
